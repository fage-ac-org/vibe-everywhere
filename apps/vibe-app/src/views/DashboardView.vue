...
const activeSection = computed(
  () =>
    dashboardSections.find((item) => item.routeName === route.name) ??
    dashboardSections[0],
);
...
<template>
  <main
    class="mx-auto flex min-h-screen w-full max-w-[1800px] gap-4 px-3 py-4 md:px-6 xl:px-8"
  >
    <aside
    ...
    <section class="min-w-0 flex-1 pb-24 lg:pb-6">
      ...
      >
        <div class="space-y-2">
          <Badge
            variant="outline"
            class="border-border/70 bg-background/55 text-foreground"
          >
