...
import {
  detectControlClientKind,
  prefersExplicitRemoteRelayUrl,
} from "@/lib/platform";
import {
...
const DASHBOARD_CONTROLLER_KEY: InjectionKey<DashboardController> = Symbol(
...
export function createDashboardController() {
  ...
  const selectedDeviceAvailableProviders = computed(
    () =>
      selectedDevice.value?.providers.filter(
  ...
  const canCreatePortForward = computed(() => {
    const targetPort = Number.parseInt(
      portForwardDraft.value.targetPort.trim(),
      10,
    );
    return (
  ...
  const canClosePortForward = computed(() =>
    Boolean(
      ...
      !["closed", "failed", "close_requested"].includes(
        selectedPortForward.value.status,
      ),
    ),
  );
  ...
  const activePlatformCapability = computed(
    () =>
      platformMatrix.value.find(
        (capability) => capability.client === currentClientKind.value,
      ) ?? null,
  );
  const currentActor = computed(() => appConfig.value?.currentActor ?? null);
  const showMobileRelayHint = computed(() =>
    prefersExplicitRemoteRelayUrl(appConfig.value),
  );
  const relayPlaceholder = computed(() =>
    getRelayBaseUrlPlaceholder(showMobileRelayHint.value),
  );
  const selectedDeviceSessionCount = computed(() => {
    const deviceId = selectedDevice.value?.id;
    return deviceId
      ? tasks.value.filter((task) => task.deviceId === deviceId).length
      : 0;
  });
  const selectedDeviceShellCount = computed(() => {
    const deviceId = selectedDevice.value?.id;
    return deviceId
      ? shellSessions.value.filter((session) => session.deviceId === deviceId)
          .length
      : 0;
  });
  const selectedDevicePreviewCount = computed(() => {
    const deviceId = selectedDevice.value?.id;
    return deviceId
      ? portForwards.value.filter((forward) => forward.deviceId === deviceId)
          .length
      : 0;
  });
  const selectedDeviceWorkingRoot = computed(
    () =>
      selectedDevice.value?.metadata.workingRoot ??
      t("common.useAgentWorkingRoot"),
  );
  ...
  const selectedPreviewIsReady = computed(
    () => selectedPortForward.value?.status === "active",
  );
  ...
  const showShellTools = computed(
    () =>
      !appConfig.value ||
      hasAppFeatureFlag(appConfig.value, APP_FEATURE_FLAGS.relayShellSessions),
  );
  const showPreviewTools = computed(
    () =>
      ...
      hasAppFeatureFlag(
        appConfig.value,
        APP_FEATURE_FLAGS.relayTcpForwardingControlPlane,
      ),
  );
  const showGitInspect = computed(
    () =>
      !appConfig.value ||
      hasAppFeatureFlag(appConfig.value, APP_FEATURE_FLAGS.sessionGitInspect),
  );
  ...
  const deploymentDocsUrl = computed(
    () => appConfig.value?.deployment.documentationUrl ?? "",
  );
  const sessionWorkspaceTitle = computed(() => {
    ...
    if (selectedDevice.value) {
      return t("dashboard.sessions.newOnDevice", {
  ...
  const workspaceSessionCwd = computed(() => {
    if (selectedTask.value?.cwd) {
      return selectedTask.value.cwd;
    }

    const configuredProjectFolder = projectFolder.value.trim();
    return configuredProjectFolder || undefined;
  });
  const sessionEventCounts = computed(() => {
    const events = selectedTaskDetail.value?.events ?? [];
    return {
      assistant: events.filter((event) => event.kind === "assistant_delta")
        .length,
      tool: events.filter(
  ...
  const visibleChangedFileCount = computed(() =>
    gitInspect.value?.state === "ready"
      ? gitInspect.value.changedFiles.length
      : 0,
  );
  const supervisionSummary = computed(() => {
    ...
    if (
      ["pending", "assigned", "running", "cancel_requested"].includes(
        taskStatus,
      )
    ) {
    ...
    return changed
      ? t("dashboard.workspace.supervision.summary.failedWithChanges", {
          changed,
        })
  ...
  function formatShellStatus(value: string, closeRequested: boolean) {
    const effectiveValue =
      closeRequested && value === "active" ? "close_requested" : value;
  ...
  function deviceShellCount(deviceId: string) {
    return shellSessions.value.filter(
      (session) => session.deviceId === deviceId,
    ).length;
  }

  function devicePreviewCount(deviceId: string) {
    return portForwards.value.filter((forward) => forward.deviceId === deviceId)
      .length;
  }
    ...
    try {
      ...
      if (
        ...
        !response.entries.some(
          (entry) => entry.path === selectedWorkspacePath.value,
        )
      ) {
        selectedWorkspacePath.value = null;
        workspacePreview.value = null;
  ...
  async function navigateWorkspaceRoot() {
    await loadWorkspace();
  }
...
function statusBadgeClass(
  kind: "online" | "offline" | "running" | "warning" | "done" | "error",
) {
  switch (kind) {
    case "online":
...
function activitySeverityClass(
