...
Vibe Everywhere 是面向自建环境的远程 AI 开发工作台。系统由 `vibe-relay`、`vibe-agent` 和控制端组成，用于在远程主机上运行长期 AI 开发会话，并围绕 `服务器 -> 主机 -> 项目 -> 会话/任务执行` 这一主模型提供会话、变更、文件三个核心工作面。
...
| `vibe-agent` | 运行在目标主机上；负责执行 Provider CLI、工作区访问、Git 检查和项目运行时 | 需要执行 AI 任务的目标机器 |
...
- 工作区目录浏览和文本文件预览
- Git 状态、变更文件列表、最近提交检查和文件级 diff 审查
- 会话中按任务展示的摘要、执行事件和必要输出
- 会话内直接停止运行中任务
- 会话卡片中的快速后续动作：重试任务、解释结果、查看变更
...
1. 打开桌面端、Android 客户端或自建 Web 客户端。
2. 在服务器设置里输入 relay 地址和 `VIBE_RELAY_ACCESS_TOKEN`。
...
| `VIBE_PUBLIC_RELAY_BASE_URL` | 客户端访问 relay 时使用的对外地址 | 无公网默认值 | 用于客户端连接信息和对外链接生成 |

关键规则：

- `VIBE_PUBLIC_RELAY_BASE_URL` 不会改变 relay 实际监听的端口。
- 如果 relay 监听在 `8787`，客户端也直接连这个端口，那么该值必须包含 `:8787`。
- `0.0.0.0` 只能作为监听地址，不能直接作为客户端访问地址。
- `127.0.0.1` 和 `localhost` 仅适用于同机本地开发。
