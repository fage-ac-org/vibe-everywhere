...
- The README/developer-guide split does not change runtime behavior. It only clarifies the document
  entry points for operators versus contributors.
...
- GitHub Actions `CI` run `#21` completed successfully on `main` before the release tag was cut.
